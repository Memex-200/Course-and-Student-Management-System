...
const CafeteriaReports: React.FC = () => {
  //const { user } = useAuth();
  const [statistics, setStatistics] = useState<CafeteriaStatistics | null>(null);
  ...
  const exportToExcel = async () => {
    try {
      ...
      link.href = url;
      link.download = `cafeteria-report-${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const exportToPDF = async () => {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `cafeteria-report-${new Date().toISOString().split('T')[0]}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  هامش ربح {((statistics.totalProfit / statistics.totalRevenue) * 100).toFixed(1)}%
                </p>
              </div>
            ...
            <LineChart data={revenueData}>
              ...
              <Tooltip 
                formatter={(value, name) => [
                  `${value} ${name === 'revenue' ? 'جنيه' : name === 'orders' ? 'طلب' : 'جنيه'}`,
                  name === 'revenue' ? 'الإيرادات' : name === 'orders' ? 'الطلبات' : 'الربح'
                ]}
              />
              <Line 
                type="monotone" 
          ...
          <RechartsBarChart data={revenueData}>
            ...
            <Tooltip 
              formatter={(value, name) => {
                ...
                if (name === 'عدد الطلبات') return [`${value} طلب`, 'عدد الطلبات'];
                return [value, name];
              }}
            />
            <Legend />
            <Bar yAxisId="left" dataKey="revenue" fill="#8884d8" name="الإيرادات" />
            <Bar yAxisId="right" dataKey="orders" fill="#82ca9d" name="عدد الطلبات" />
          </RechartsBarChart>
        </ResponsiveContainer>
      </div>
              ...
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{item.quantity} قطعة</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-gray-900">{item.sales} جنيه</p>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {statistics && [
                { metric: "إجمالي الطلبات", value: statistics.totalOrders, comparison: "+8%" },
                { metric: "الطلبات المكتملة", value: statistics.completedOrders, comparison: "+12%" },
                { metric: "الطلبات الملغية", value: statistics.cancelledOrders, comparison: "-5%" },
                { metric: "معدل الإلغاء", value: `${((statistics.cancelledOrders / statistics.totalOrders) * 100).toFixed(1)}%`, comparison: "-2%" },
                { metric: "إجمالي الإيرادات", value: `${statistics.totalRevenue.toLocaleString()} جنيه`, comparison: "+15%" },
                { metric: "إجمالي التكاليف", value: `${statistics.totalCost.toLocaleString()} جنيه`, comparison: "+8%" },
                { metric: "صافي الربح", value: `${statistics.totalProfit.toLocaleString()} جنيه`, comparison: "+22%" },
                { metric: "هامش الربح", value: `${((statistics.totalProfit / statistics.totalRevenue) * 100).toFixed(1)}%`, comparison: "+3%" },
              ].map((row, index) => (
                <tr key={index}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`${row.comparison.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                      {row.comparison}
                    </span>
                  </td>
                </tr>
